...
// flexible_launch_configs takes int and runs ceil_div on its arguments, so keep
// them far enough below INT_MAX that the intermediate cannot overflow. The
// configuration it returns saturates long before this bound.
constexpr int64_t kMaxLaunchConfigExtent =
...
__global__ void ComputeInternalGradientsChannelsLastCUDAKernel(
  ...
  for (int64_t n = blockIdx.z; n < N; n += gridDim.z) {
    ...
    if (gridDim.y == 1) {
      ...
      // See the matching comment in RowwiseMomentsChannelsLastCUDAKernel.
      __syncthreads();
      continue;
    }
  ...
  if (channels_last) {
    if (D == 1) {
      ...
      flexible_launch_configs(
          std::min<int64_t>(HxW, kMaxLaunchConfigExtent),
          std::min<int64_t>(C, kMaxLaunchConfigExtent),
          block,
          grid,
          /*coop_flag=*/true);
      grid.z = std::min<int64_t>(N, kMaxGridZ);
      // mean and m2n as T_ACC, then the counts as int64_t, which occupy two
      // T_ACC slots each.
      Tensor staging_data;
      Tensor semaphores;
      if (grid.y > 1) {
